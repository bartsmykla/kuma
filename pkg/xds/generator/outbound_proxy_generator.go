...
func (g OutboundProxyGenerator) Generate(ctx xds_context.Context, proxy *model.Proxy) (*model.ResourceSet, error) {
	...
	// serviceAcc:
	//   dataplane-4       (dataplane-4_0_)
	//   service_in_zone-2 (service_in_zone-2_1_, service_in_zone-2)
	//   externalservice-1 (externalservice-1_2_)
		...
		for _, cluster := range service.Clusters() {
			...
			if service.HasExternalService() {
				if len(ctx.Mesh.Resources.ZoneEgresses().Items) > 0 {
					edsClusterBuilder.
						Configure(envoy_clusters.EdsCluster(cluster.Service())).
				...
				edsClusterBuilder.
					...
					Configure(envoy_clusters.ClientSideMTLS(ctx, name, tlsReady, clusterTags)).
