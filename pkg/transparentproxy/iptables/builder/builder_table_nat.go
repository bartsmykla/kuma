...
func addOutputRules(
	...
	for _, uIDsToPorts := range cfg.Redirect.Outbound.ExcludePortsForUIDs {
		...
		default:
			...
			// TODO(bartsmykla): there should probably be added prefix with IP
			//  type (IPv4 | IPv6)
			l.Warnf("unknown protocol %s, only 'tcp' or 'udp' allowed", uIDsToPorts.Protocol)
			continue
		}
