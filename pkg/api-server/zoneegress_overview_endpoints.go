...
func (r *zoneEgressOverviewEndpoints) addFindEndpoint(ws *restful.WebService) {
	ws.Route(ws.GET("/zoneegress-overviews/{name}").To(r.inspectZoneEgress).
...
func (r *zoneEgressOverviewEndpoints) addListEndpoint(ws *restful.WebService) {
	ws.Route(ws.GET("/zoneegress-overviews").To(r.inspectZoneEgresses).
		Doc("Inspect all zone egresses").
		Returns(200, "OK", nil))
}

func (r *zoneEgressOverviewEndpoints) inspectZoneEgress(request *restful.Request, response *restful.Response) {
...
func (r *zoneEgressOverviewEndpoints) fetchOverview(ctx context.Context, name string) (*mesh.ZoneEgressOverviewResource, error) {
...
func (r *zoneEgressOverviewEndpoints) inspectZoneEgresses(request *restful.Request, response *restful.Response) {
...
func (r *zoneEgressOverviewEndpoints) fetchOverviews(ctx context.Context, p page) (mesh.ZoneEgressOverviewResourceList, error) {
	...
	// we cannot paginate insights since there is no guarantee that the elements will be the same as zone egresses
